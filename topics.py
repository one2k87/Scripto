...
- ❌ '블로그·수익·마케팅' 자체를 주제로 삼지 마라(2026-09-17 실측 사고). 이 지시문에
  '수익형 블로그'라는 말이 있다고 해서 그것이 글감이 되는 게 아니다. 독자는 블로그를
  운영하려는 사람이 아니라 '{category}'의 문제를 겪는 당사자다.
  (나쁜 예: "시니어 디지털 블로그 수익 주제 3가지", "애드센스 승인받는 법",
   "조회수 늘리는 키워드 찾기" → 전부 금지)
...
# 블로그 운영·수익화 자체를 다루는 '메타 주제' 차단(2026-09-17 실측).
# 주제 프롬프트가 "애드센스 수익형 블로그의 주제 기획자"로 시작하는 탓에, 후보가
# 빈약한 날 모델이 그 틀 자체를 주제로 써버렸다("시니어 디지털 블로그 수익 주제 3가지").
# 독자는 시니어와 그 자녀지 블로거가 아니다 — 심사에서 '가치 없는 콘텐츠'로 직행하는 유형.
META_HINTS = [
    "블로그 수익", "블로그 주제", "블로그 운영", "블로그 시작", "블로그로 돈",
    "애드센스", "구글 광고 수익", "제휴 마케팅", "쿠팡 파트너스", "수익률 비교",
    "수익화", "부업으로", "온라인 부업", "재택 부업", "조회수 늘리", "방문자 늘리",
    "트래픽 늘리", "키워드 발굴", "상위노출 하는 법", "SEO 하는 법", "콘텐츠 마케팅",
]


def is_meta_topic(keyword):
    kw = str(keyword or "")
    return any(h and h in kw for h in META_HINTS)
