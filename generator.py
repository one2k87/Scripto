...
def _article_prompt(keyword, kind, category, links, related, insert_ads, competitive=False, intent_ctx=""):
    ...
    ad_rule += (
        ...
        "   복잡한 화면 그림을 클릭하지 않습니다. 사람의 표정·손·공간·사물로 상황을 전하세요.\n"
        f"② 설명 이미지(본문 중반): 스타일 {_sec_style} → {_sec_desc}\n"
        "   이 글의 내용에 맞게 구체화해 한국어 1~2문장으로 쓰세요.\n"
        "[공통] 형식 [[IMG:스타일|장면묘사]]. 장면묘사에는 이 글의 핵심 소재를 그대로 포함하고,\n"
        "글과 무관한 배경·인물·풍경은 넣지 마세요.\n"
        "[글자] 버튼 이름·단계 이름 같은 '짧은 한글 라벨'은 넣어도 됩니다(생성 후 자동 검수가\n"
        "깨진 글자를 걸러 다시 만듭니다). 다만 문장·문단·긴 목록은 넣지 마세요 — 깨집니다.\n"
...
1. 실전 신호 2개 이상 — 다음 중에서(검수기가 이 표현 계열을 기계로 찾으니 자연스럽게 꼭 포함):
   자주 나는 실수·실패("~를 잘못 누르면", "흔한 실수", "자주 헷갈리는"), 조건 분기("~인 경우에는",
   "만약 ~라면", "기기·버전에 따라 다를 수 있"), 두 방법·선택지의 장단 비교("반대로", "차이가"),
   경고("주의할 점", "여기서 멈추고", "~는 하지 마세요"), 안 될 때 대처("안 될 때는").
   ※ 겪지 않은 1인칭 일화("제가 직접 해보니")를 지어내는 건 금지 —
   '흔히 겪는 문제' 관점으로 쓰면 거짓말 없이 같은 효과가 난다.
2. 실측형 수치 3개 이상 — 이 주제에 맞는 단위로: 가격·요금(원, 기준 시점 표기), 소요 시간(분),
   단계 수, 용량·개수(GB·개), 규격. 확실치 않으면 범위로("월 3만~5만 원대", "5~10분").
...
HERO_PLOTS = [
    ("손안의 순간", "", "어르신의 손이 스마트폰을 들고 있는 클로즈업. 화면은 은은하게 빛나고, 따뜻한 실내 채광에 주름과 온기가 느껴지는 손"),
    ("나란히 보기", "", "자녀와 부모가 소파나 식탁에 나란히 앉아 한 화면을 함께 들여다보는 장면. 알려주는 쪽은 차분하고 배우는 쪽은 집중한 표정, 편안한 거실 분위기"),
    ("책상 위 정황", "", "식탁이나 거실 탁자 위에 스마트폰·돋보기·메모지·따뜻한 차가 놓인 정물. 사람 없이 사물만으로 상황을 짐작하게"),
    ("전화 앞에서", r"전화|통화|보이스|피싱|사기|스팸|문자|메시지|번호", "전화를 받고 있는 어르신의 옆모습. 미간에 망설임이 스친 신중한 표정, 다른 손은 탁자 위에 멈춰 있고 배경은 단순한 집 안"),
    ("메모하는 손", r"전화|통화|상담|접수|신청|문의|고객센터|예약", "한 손으로 전화기를 귀에 대고 다른 손으로 종이에 받아 적는 어르신. 집중한 표정, 단정한 책상 위"),
    ("누르기 직전", r"설정|바꾸|변경|켜|끄|글씨|크기|화면|버튼|누르|삭제|차단|등록|가입|해지", "스마트폰 화면 앞에서 검지가 버튼을 누르려는 순간의 클로즈업. 화면의 버튼은 큼직하게, 손끝에 초점"),
    ("창가의 영상통화", r"영상통화|사진|앨범|카톡|카카오|가족|손주|공유|전송|보내", "창가에서 스마트폰을 세워 들고 화면 속 가족을 보며 환하게 웃는 어르신. 자연광이 얼굴에 부드럽게 닿고, 화면 속 인물은 흐릿하게"),
    ("기계 앞에 서서", r"키오스크|무인|단말|주문|발급|창구|은행|ATM|매장|병원|주민센터|정부24|인증", "무인 기기나 매장 단말기 앞에 선 어르신. 화면을 올려다보며 무엇을 눌러야 할지 고민하는 모습, 공간감이 느껴지는 넓은 구도"),
    ("서류와 돋보기", r"요금|통신비|청구|고지서|서류|신청서|증명|등본|비용|할인|납부", "탁자 위 고지서를 돋보기로 들여다보는 어르신. 눈을 가늘게 뜨고 한 줄을 짚어가는 모습"),
]
# 두 번째(설명) 이미지 역할도 돌려쓴다 — 매번 같은 도해가 나오지 않게
SECOND_ROLES = [
    ("diagram", "이 글의 절차를 3~5단계 흐름으로 보여주는 도해. 각 단계에 짧은 한글 라벨(두세 글자)을 큼직하게"),
    ("diagram", "이 글에서 눌러야 할 스마트폰 화면을 크게 그린 그림. 큰 버튼 몇 개와 그 위의 짧은 한글 이름, 눌러야 할 곳을 원형 표시로"),
    ("object", "이 글의 핵심 사물 클로즈업(유심칩·카드·단말기·고지서 등 주제에 맞는 것 하나)"),
    ("diagram", "두 가지 선택지를 좌우로 나란히 비교하는 도해. 각 쪽에 짧은 한글 제목 한 단어씩"),
    ("photo", "어르신이 실제로 그 화면을 켠 스마트폰을 손에 들고 있는 사진. 화면 속 큰 버튼이 또렷하게 보이도록"),
